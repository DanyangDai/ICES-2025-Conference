---
title: "Offcial and Online Infation Spillover analysis of the Euro Area"
author:
  - Danyang Dai, The University of Queensland
format: 
  revealjs:
    multiplex: true
    footer: "[]()"
...
-   Inflation is directly linked to the determination of domestic interest rate and maintaining purchasing power.

-  Central bank: Controlling the economic activity and performance
...
-  Provide "early warning systems" for emergent crises and track the progress of existing crises. 

- Allows for network analysis: how market and countries' economy are connected. 
...
- Among the 9 countries, including France, Germany Spain, Italy, the UK, Sweden, Swizerland Denmark and Norway, **France and Germany** were net transmitter of spillovers.
...
## Backgroun - Official vs Online Price Index
...
- This research aim to evaluate the speed and magnitude of the spillover between the official inflation vs the online inflation. 

## Data 


- The data used in this project were from Aparicio and Bertolotto (2020) where they used the online prices for forecasting inflation. 
...
- Range of the data covered from July 2008 to September 2020.
...
## Data - Offical Inflation and Online Inflation 
...
## Method: Seasonal Trend Loess Decomposition - Offcial CPI
...
The spillover measurement is based on forecast error variance decomposition (FEVD) from vector autoregressive (VAR) model (Diebold and Yilmaz,2009).

For this study, VAR(1) was selected based on information selection criterion.
...
$$
where $Y_{t}$ is a vector containing all the inflation indices from the selected counties. $A_{1}$ is a 7 by 7 matrix. 

## Method - Vector Moving Average 

Re-write the equation into moving average representation:
...
- Aggarwal and Goodell (2010) suggests that one possible way of ranking the national  financial influence is to use the ratio of domestic stock market capitalisation
to GDP over domestic assets of deposit many to bank capitalisation to GDP.

- Rank chosen to be: the UK, France, Netherlands, Ireland, Italy, Greece and Germany. 

## Method - FEVD

Forecast Error Variance Deposition was calculated by feeding a shock to the UK at one, four and twelve months.

The individual shock contribution is defined as the contribution of the $i^{th}$ to the mean square forecast error (MSFE) of the $n^{th}$ variable $y_{n, t+h} h$ period ahead is:

$$
\frac{MSFE^{n}_{i}(h)}{\sum^{N}_{i=1}MSFE^{n}_{i}(h)}
$$

Because the shocks to each variable are othogonalised, the sum of the contribution to the variance of the forecast error is equal to 1. 

## Method - DY Spillover 


The DY spillover is given:

$$
Spillover = \frac{\sum^{N}_{i=1}\sum^{N}_{j=1}Var(\mu_{t+h}|t)_{ij}}{100N}
$$

where $N$ is equal to s7 as there are 7 countries in the system. 
...
- For Germany, one-period after the shocked fed to the system, only 28.7% of the forecast error variance was explained by itself.

- Compared to the official inflation, inflation index conducted with online prices were less influenced by other countries. 
...
- The UK's official inflation is still largel self-determined. 
...
- The official inflation has higher magnitude in transmitting volatility than online inflation indices.
...
- Online prices are collected from a relatively small set of retailers and product categories than the comprehensive consumer prices survey conducted by the government (Cavallo and Rigobon, 2016).

## Limtation 

- Diebold and Yilmaz (2012) suggests that the Cholesky ordering used in this study may have an impact on the variance decomposition. 
...
- Acknowledgement: this work was done under the supervision of Professor Kalvinder Shiel and Dr.Andrew Clarke as part of Master of Applied Econometics degree at the University of Melbourne. 
